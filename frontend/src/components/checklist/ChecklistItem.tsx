...
import { Camera, Check, X, Flame } from 'lucide-react'
...
export function ChecklistItem({ habit, completed, onProofRequest, onUncheck, streak = 0 }: ChecklistItemProps) {
  return (
    <button
      onClick={completed ? onUncheck : onProofRequest}
      className={cn(
        'w-full flex items-center gap-4 px-4 py-4 rounded-2xl border transition-all duration-300 text-left group',
        completed
          ? 'bg-[#EAF3EC] border-[#A8C4B0] hover:bg-[#F5EEE8] hover:border-[#C9A96E] cursor-pointer'
    ...
    >
      ...
      <div
        className={cn(
          ...
          completed
            ? 'bg-[#1E3D2F] group-hover:bg-[#B5534D]'
            : 'border-2 border-[#D5CEC5] group-hover:border-[#C9A96E] group-hover:bg-[#FFF9F0]',
        )}
      >
        {completed
          ? <>
              <Check size={14} className="text-white group-hover:hidden" strokeWidth={3} />
              <X size={14} className="text-white hidden group-hover:block" strokeWidth={3} />
            </>
